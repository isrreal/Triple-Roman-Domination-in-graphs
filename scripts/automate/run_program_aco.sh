...
output_directory="../output_files/ant_colony_optimization_outputs/atmost333_outputs/$output_folder_name"
...
for input_file in "$input_directory"/*.txt; do
    if [ -f "$input_file" ]; then
        ...
        $executable "$input_file" "$(basename "$input_file" .txt)" "$heuristic" 1 "$rvns" > "$output_file"
