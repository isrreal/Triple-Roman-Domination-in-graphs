...
output_directory="../output_files/genetic_algorithm_outputs/$input_folder_name/$output_folder_name"
...
for input_file in "$input_directory"/*.txt; do
    if [ -f "$input_file" ]; then
        ...
        $executable "$input_file" "$(basename "$input_file" .txt)" "$heuristic" 0 > "$output_file"
